...
class ArbDessin extends Canvas{
     ...
     public void dessinerSousArbre
       (Graphics g,ArbreElem tree, int x, int width,int level) {
       ...
       g.drawString(tree.getEtiquette(),x-(int)(w/2),(level*V)+(int)(h/4));
       if (tree.getGauche()!=null){
	 g.drawLine(x-RADIUS,level*V,x-(int)(width/4),(level+1)*V);
	 dessinerSousArbre(g,tree.getGauche(),x-(int)(width/4),(int)(width/2),level+1);
       }
       if (tree.getDroite()!=null){
	 g.drawLine(x+RADIUS,level*V,x+(int)(width/4),(level+1)*V);
	 dessinerSousArbre(g, tree.getDroite(), x+(int)(width/4), (int)(width/2), level+1);
       }
     }
   }